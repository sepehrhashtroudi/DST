...
from keras.layers import Convolution1D, RepeatVector, Bidirectional,Input, Embedding, LSTM, Dense,concatenate,CuDNNLSTM
...
lstm_out = Bidirectional(CuDNNLSTM(units=150, return_sequences=False , stateful = False))(x)
...
# bs_out_food_joined = Bidirectional(LSTM(units=300, return_sequences=False , stateful = True))(bs_out_food_joined)
...
# bs_input_area = Dense(area_nb_classes, activation='relu')(bs_input_area)
bs_out_area_joined = concatenate([lstm_out, bs_input_area])
# bs_out_area_joined = RepeatVector(1)(bs_out_area_joined)

# bs_out_area_joined = Bidirectional(CuDNNLSTM(units=300, return_sequences=False , stateful = True))(bs_out_area_joined)
bs_out_area_joined = Dense(area_nb_classes, activation='relu')(bs_out_area_joined)
bs_out_area_joined = Dense(area_nb_classes, activation='relu')(bs_out_area_joined)
...
model = Model(inputs=[sent_input, bs_input_area], outputs=[bs_out_area_joined])
    ...
    for n_batch, dialog in enumerate(tqdm(dialogs)):
        for n_turn, turn in enumerate(dialog):
            ...
            if sent.shape[1] > 1:  # some bug in keras
                ...
                loss = model.train_on_batch([sent,area_turn_bs_in], [area_out])
...
#             food_bs_pred = np.argmax(pred, -1)[0]
            ...
            ground_truth.append([np.argmax(area_out, -1)[0]])
...
#     food_acc = accuracy_score(np.array(ground_truth)[:, 0], np.array(bs_pred)[:, 0])
    area_acc = accuracy_score(np.array(ground_truth)[:, 0], np.array(bs_pred)[:, 0])
...
#     food_acc = accuracy_score(np.array(test_ground_truth)[:, 0], np.array(test_bs_pred)[:, 0])
    area_acc = accuracy_score(np.array(test_ground_truth)[:, 0], np.array(test_bs_pred)[:, 0])
